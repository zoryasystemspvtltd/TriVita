...
public sealed class ReportLockStateControllerTests
{
    private readonly Mock<ILisReportLockStateService> _service = new();

    private ReportLockStateController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<ReportLockStateController>.Instance)
    {
    ...
    {
        var dto = new ReportLockStateResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<ReportLockStateResponseDto>>.Ok(new PagedResponse<ReportLockStateResponseDto>
            {
                Items = Array.Empty<ReportLockStateResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new ReportLockStateResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateReportLockStateDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateReportLockStateDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateReportLockStateDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new ReportLockStateResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateReportLockStateDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateReportLockStateDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateReportLockStateDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportLockStateResponseDto>.Fail("Conflict"));
