...
public sealed class LabOrderItemControllerTests
{
    private readonly Mock<ILisLabOrderItemService> _service = new();

    private LabOrderItemController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<LabOrderItemController>.Instance)
    {
    ...
    {
        var dto = new LabOrderItemResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<LabOrderItemResponseDto>>.Ok(new PagedResponse<LabOrderItemResponseDto>
            {
                Items = Array.Empty<LabOrderItemResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new LabOrderItemResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabOrderItemDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateLabOrderItemDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabOrderItemDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new LabOrderItemResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabOrderItemDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateLabOrderItemDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabOrderItemDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderItemResponseDto>.Fail("Conflict"));
