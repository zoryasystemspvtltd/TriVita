...
public sealed class ResultApprovalControllerTests
{
    private readonly Mock<ILisResultApprovalService> _service = new();

    private ResultApprovalController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<ResultApprovalController>.Instance)
    {
    ...
    {
        var dto = new ResultApprovalResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<ResultApprovalResponseDto>>.Ok(new PagedResponse<ResultApprovalResponseDto>
            {
                Items = Array.Empty<ResultApprovalResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new ResultApprovalResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateResultApprovalDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateResultApprovalDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateResultApprovalDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new ResultApprovalResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateResultApprovalDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateResultApprovalDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateResultApprovalDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultApprovalResponseDto>.Fail("Conflict"));
