...
public sealed class LabOrderControllerTests
{
    private readonly Mock<ILisLabOrderService> _service = new();

    private LabOrderController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<LabOrderController>.Instance)
    {
    ...
    {
        var dto = new LabOrderResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<LabOrderResponseDto>>.Ok(new PagedResponse<LabOrderResponseDto>
            {
                Items = Array.Empty<LabOrderResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new LabOrderResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabOrderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateLabOrderDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabOrderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new LabOrderResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabOrderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateLabOrderDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabOrderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabOrderResponseDto>.Fail("Conflict"));
