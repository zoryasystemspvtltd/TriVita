...
public sealed class ResultHistoryControllerTests
{
    private readonly Mock<ILisResultHistoryService> _service = new();

    private ResultHistoryController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<ResultHistoryController>.Instance)
    {
    ...
    {
        var dto = new ResultHistoryResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<ResultHistoryResponseDto>>.Ok(new PagedResponse<ResultHistoryResponseDto>
            {
                Items = Array.Empty<ResultHistoryResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new ResultHistoryResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateResultHistoryDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateResultHistoryDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateResultHistoryDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new ResultHistoryResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateResultHistoryDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateResultHistoryDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateResultHistoryDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ResultHistoryResponseDto>.Fail("Conflict"));
