...
public sealed class SampleTrackingControllerTests
{
    private readonly Mock<ILisSampleTrackingService> _service = new();

    private SampleTrackingController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<SampleTrackingController>.Instance)
    {
    ...
    {
        var dto = new SampleTrackingResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<SampleTrackingResponseDto>>.Ok(new PagedResponse<SampleTrackingResponseDto>
            {
                Items = Array.Empty<SampleTrackingResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new SampleTrackingResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleTrackingDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateSampleTrackingDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleTrackingDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new SampleTrackingResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleTrackingDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateSampleTrackingDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleTrackingDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTrackingResponseDto>.Fail("Conflict"));
