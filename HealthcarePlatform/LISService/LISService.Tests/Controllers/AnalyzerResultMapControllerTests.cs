...
public sealed class AnalyzerResultMapControllerTests
{
    private readonly Mock<ILisAnalyzerResultMapService> _service = new();

    private AnalyzerResultMapController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<AnalyzerResultMapController>.Instance)
    {
    ...
    {
        var dto = new AnalyzerResultMapResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<AnalyzerResultMapResponseDto>>.Ok(new PagedResponse<AnalyzerResultMapResponseDto>
            {
                Items = Array.Empty<AnalyzerResultMapResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new AnalyzerResultMapResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateAnalyzerResultMapDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateAnalyzerResultMapDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateAnalyzerResultMapDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new AnalyzerResultMapResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateAnalyzerResultMapDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateAnalyzerResultMapDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateAnalyzerResultMapDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<AnalyzerResultMapResponseDto>.Fail("Conflict"));
