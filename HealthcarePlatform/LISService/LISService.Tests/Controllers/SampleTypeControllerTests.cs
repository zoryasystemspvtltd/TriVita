...
public sealed class SampleTypeControllerTests
{
    private readonly Mock<ILisSampleTypeService> _service = new();

    private SampleTypeController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<SampleTypeController>.Instance)
    {
    ...
    {
        var dto = new SampleTypeResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<SampleTypeResponseDto>>.Ok(new PagedResponse<SampleTypeResponseDto>
            {
                Items = Array.Empty<SampleTypeResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new SampleTypeResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleTypeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateSampleTypeDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleTypeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new SampleTypeResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleTypeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateSampleTypeDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleTypeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleTypeResponseDto>.Fail("Conflict"));
