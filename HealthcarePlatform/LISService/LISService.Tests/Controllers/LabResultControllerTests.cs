...
public sealed class LabResultControllerTests
{
    private readonly Mock<ILisLabResultService> _service = new();

    private LabResultController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<LabResultController>.Instance)
    {
    ...
    {
        var dto = new LabResultResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<LabResultResponseDto>>.Ok(new PagedResponse<LabResultResponseDto>
            {
                Items = Array.Empty<LabResultResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new LabResultResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabResultDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateLabResultDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateLabResultDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new LabResultResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabResultDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateLabResultDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateLabResultDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<LabResultResponseDto>.Fail("Conflict"));
