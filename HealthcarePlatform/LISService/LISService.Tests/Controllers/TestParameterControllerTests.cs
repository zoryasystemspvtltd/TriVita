...
public sealed class TestParameterControllerTests
{
    private readonly Mock<ILisTestParameterService> _service = new();

    private TestParameterController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<TestParameterController>.Instance)
    {
    ...
    {
        var dto = new TestParameterResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<TestParameterResponseDto>>.Ok(new PagedResponse<TestParameterResponseDto>
            {
                Items = Array.Empty<TestParameterResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new TestParameterResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateTestParameterDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateTestParameterDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateTestParameterDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new TestParameterResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateTestParameterDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateTestParameterDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateTestParameterDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterResponseDto>.Fail("Conflict"));
