...
public sealed class ReportHeaderControllerTests
{
    private readonly Mock<ILisReportHeaderService> _service = new();

    private ReportHeaderController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<ReportHeaderController>.Instance)
    {
    ...
    {
        var dto = new ReportHeaderResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<ReportHeaderResponseDto>>.Ok(new PagedResponse<ReportHeaderResponseDto>
            {
                Items = Array.Empty<ReportHeaderResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new ReportHeaderResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateReportHeaderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateReportHeaderDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateReportHeaderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new ReportHeaderResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateReportHeaderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateReportHeaderDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateReportHeaderDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<ReportHeaderResponseDto>.Fail("Conflict"));
