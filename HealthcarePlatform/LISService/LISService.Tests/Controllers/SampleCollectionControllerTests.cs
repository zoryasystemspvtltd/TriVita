...
public sealed class SampleCollectionControllerTests
{
    private readonly Mock<ILisSampleCollectionService> _service = new();

    private SampleCollectionController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<SampleCollectionController>.Instance)
    {
    ...
    {
        var dto = new SampleCollectionResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<SampleCollectionResponseDto>>.Ok(new PagedResponse<SampleCollectionResponseDto>
            {
                Items = Array.Empty<SampleCollectionResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new SampleCollectionResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleCollectionDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateSampleCollectionDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleCollectionDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new SampleCollectionResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleCollectionDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateSampleCollectionDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleCollectionDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleCollectionResponseDto>.Fail("Conflict"));
