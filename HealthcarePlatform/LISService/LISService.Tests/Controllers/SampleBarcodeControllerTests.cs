...
public sealed class SampleBarcodeControllerTests
{
    private readonly Mock<ILisSampleBarcodeService> _service = new();

    private SampleBarcodeController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<SampleBarcodeController>.Instance)
    {
    ...
    {
        var dto = new SampleBarcodeResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<SampleBarcodeResponseDto>>.Ok(new PagedResponse<SampleBarcodeResponseDto>
            {
                Items = Array.Empty<SampleBarcodeResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new SampleBarcodeResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleBarcodeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateSampleBarcodeDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateSampleBarcodeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new SampleBarcodeResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleBarcodeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateSampleBarcodeDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateSampleBarcodeDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<SampleBarcodeResponseDto>.Fail("Conflict"));
