...
public sealed class TestParameterProfileControllerTests
{
    private readonly Mock<ILisTestParameterProfileService> _service = new();

    private TestParameterProfileController CreateController() => new(
        _service.Object,
        LisStandardCrudControllerTestTemplate.TenantMock().Object,
        NullLogger<TestParameterProfileController>.Instance)
    {
    ...
    {
        var dto = new TestParameterProfileResponseDto { Id = 1 };
        _service.Setup(s => s.GetByIdAsync(1, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Ok(dto));
    ...
    public async Task GetById_Should_Return_Ok_With_Error_BaseResponse_When_Not_Found()
    {
        _service.Setup(s => s.GetByIdAsync(99, It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Fail("Not found"));
    ...
    public async Task GetPaged_Should_Return_Ok_When_Valid()
    {
        _service.Setup(s => s.GetPagedAsync(It.IsAny<PagedQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<PagedResponse<TestParameterProfileResponseDto>>.Ok(new PagedResponse<TestParameterProfileResponseDto>
            {
                Items = Array.Empty<TestParameterProfileResponseDto>(),
                Page = 1,
                PageSize = 10,
                TotalCount = 0
            }));
    ...
    {
        var created = new TestParameterProfileResponseDto { Id = 2 };
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateTestParameterProfileDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Ok(created));

        var result = await CreateController().Create(new CreateTestParameterProfileDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.CreateAsync(It.IsAny<CreateTestParameterProfileDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Fail("Invalid", new[] { "PatientId required" }));
    ...
    {
        var updated = new TestParameterProfileResponseDto { Id = 3 };
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateTestParameterProfileDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Ok(updated));

        var result = await CreateController().Update(3, new UpdateTestParameterProfileDto(), CancellationToken.None);
    ...
    {
        _service.Setup(s => s.UpdateAsync(3, It.IsAny<UpdateTestParameterProfileDto>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(BaseResponse<TestParameterProfileResponseDto>.Fail("Conflict"));
