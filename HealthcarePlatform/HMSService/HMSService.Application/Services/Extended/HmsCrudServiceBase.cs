...
/// <summary>
/// Generic CRUD for HMS extended entities (tenant + optional facility scoped via <see cref="RequiresFacilityId"/>).
/// </summary>
public abstract class HmsCrudServiceBase<TEntity, TCreate, TUpdate, TResponse, TService>
    ...
    where TService : class
{
    ...
    /// <summary>When true, Create requires <see cref="ITenantContext.FacilityId"/> and assigns it to the entity.</summary>
    ...
    public virtual async Task<BaseResponse<TResponse>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        Logger.LogInformation(
            "HMS {Entity} GetById tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
    ...
    {
        Logger.LogInformation(
            "HMS {Entity} GetPaged tenant {TenantId} page {Page}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            query.Page);
    ...
    public virtual async Task<BaseResponse<TResponse>> CreateAsync(TCreate dto, CancellationToken cancellationToken = default)
    {
        ...
        if (RequiresFacilityId && Tenant.FacilityId is long fid)
        {
            ...
            {
                Logger.LogWarning(
                    "HMS {Entity} Create blocked: invalid facility TenantId={TenantId} FacilityId={FacilityId}",
                    typeof(TEntity).Name,
                    Tenant.TenantId,
                    fid);
        ...
        Logger.LogInformation(
            "HMS {Entity} created tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            entity.Id);
    ...
    public virtual async Task<BaseResponse<TResponse>> UpdateAsync(long id, TUpdate dto, CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation(
            "HMS {Entity} updated tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
    ...
    public virtual async Task<BaseResponse<object?>> DeleteAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation(
            "HMS {Entity} soft-deleted tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
    ...
    /// <summary>When facility is present on the request, facility-scoped entities must match it.</summary>
