...
namespace LMSService.Application.Services.Extended;

public abstract class LmsCrudServiceBase<TEntity, TCreate, TUpdate, TResponse, TService>
    ...
    where TService : class
{
    ...
    public virtual async Task<BaseResponse<TResponse>> GetByIdAsync(long id, CancellationToken cancellationToken = default)
    {
        Logger.LogInformation(
            "LMS {Entity} GetById tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
    ...
    {
        Logger.LogInformation(
            "LMS {Entity} GetPaged tenant {TenantId} page {Page}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            query.Page);
    ...
    public virtual async Task<BaseResponse<TResponse>> CreateAsync(TCreate dto, CancellationToken cancellationToken = default)
    {
        ...
        if (RequiresFacilityId && Tenant.FacilityId is long fid)
        {
            ...
            {
                Logger.LogWarning(
                    "LMS {Entity} Create blocked: invalid facility TenantId={TenantId} FacilityId={FacilityId}",
                    typeof(TEntity).Name,
                    Tenant.TenantId,
                    fid);
        ...
        Logger.LogInformation(
            "LMS {Entity} created tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            entity.Id);
    ...
    public virtual async Task<BaseResponse<TResponse>> UpdateAsync(long id, TUpdate dto, CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation(
            "LMS {Entity} updated tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
    ...
    public virtual async Task<BaseResponse<object?>> DeleteAsync(long id, CancellationToken cancellationToken = default)
    {
        ...
        Logger.LogInformation(
            "LMS {Entity} soft-deleted tenant {TenantId} id {EntityId}",
            typeof(TEntity).Name,
            Tenant.TenantId,
            id);
        ...
        Expression<Func<TEntity, bool>> second)
    {
        ...
        var left = new LmsReplaceParameterVisitor(first.Parameters[0], param).Visit(first.Body);
        var right = new LmsReplaceParameterVisitor(second.Parameters[0], param).Visit(second.Body);
        return Expression.Lambda<Func<TEntity, bool>>(Expression.AndAlso(left!, right!), param);
    }

    private sealed class LmsReplaceParameterVisitor : ExpressionVisitor
    {
        private readonly ParameterExpression _from;
        private readonly ParameterExpression _to;

        public LmsReplaceParameterVisitor(ParameterExpression from, ParameterExpression to)
        {
            _from = from;
            _to = to;
        }
