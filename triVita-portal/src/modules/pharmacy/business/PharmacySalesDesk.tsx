...
import { getPharmacySalesPaged } from '@/services/pharmacyService';

export function PharmacySalesDesk() {
  ...
  const q = useQuery({
    queryKey: ['pharmacy', 'sales-desk', page, pageSize, searchApplied],
  ...
  return (
    <Stack spacing={3}>
      <PageHeader
        ...
        subtitle="Retail / ward sale headers — review each ticket before posting adjustments in back office."
        action={
          <TriVitaButton component={RouterLink} to="/pharmacy/data-registry?resource=pharmacy-sale" variant="outlined" size="small">
            API registry
          </TriVitaButton>
        }
      />
      <SectionContainer title="Sales tickets" subtitle="Search by sale number or patient identifiers returned by the API.">
      ...
      <DetailDrawer
        ...
        title={drawer ? `Sale ${String(drawer.saleNumber ?? drawer.id ?? '')}` : ''}
        subtitle="Header snapshot"
      >
        {drawer ? (
          <Stack spacing={1.5}>
